...
def filtro_contribuicoes(df):
    ...
    cst_invalidos = mod_55_escri_2.filter(pl.col(cd.CST).is_in(["01", "02", "03", "04", "05"]).not_())  
    ...
    resto_3 = resto_2.filter(((pl.col(cd.Registro).is_in(["C185", "C495", "C181", "C491"]))).not_())
    
    filtro_4 = resto_3.filter(
    (pl.col(cd.Registro).is_in(["C485", "C481"])) & (pl.col(cd.IND_ESCRI) == "2")
    )
   
    resto_4 = resto_3.filter(((pl.col(cd.Registro).is_in(["C485", "C481"]))).not_())
    
    filtro_5 = resto_4.filter((pl.col(cd.Registro) == "F100") & (pl.col(cd.IND_OPER).is_in(["1", "2"])))
    
    filtro_6 = resto_4.filter(((pl.col(cd.Registro) == "F100")).not_())
